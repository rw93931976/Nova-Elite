...
function App() {
  ...
  const { isListening, toggleListening: baseToggleListening, speak } = useSpeech(onProcessText);

  const toggleListening = useCallback(() => {
    if (!isListening && !hasGreeted) {
      const scaledVolume = Math.min(3.33, volume / 30);
      speak("Hi, I'm Nova. How can I help you?", scaledVolume);
      setHasGreeted(true);
      // Wait a moment for the greeting to start before opening the mic
      setTimeout(baseToggleListening, 1000);
    } else {
      baseToggleListening();
    }
  }, [isListening, hasGreeted, baseToggleListening, speak, volume]);
  ...
  useEffect(() => {
    ...
    // Level 5: Idle Research Pulse (Triggers every 2 hours)
    const interval = setInterval(() => {
      console.log("[Autonomous] Commencing scheduled study focus...");
      onProcessText("Go to school");
    }, 2 * 60 * 60 * 1000);
  ...
  return (
    <div className="w-full min-h-screen relative flex flex-col bg-transparent text-white overflow-x-hidden">
      <main className="flex-1 relative z-10 flex flex-col pt-12 px-6 pb-40 overflow-y-auto no-scrollbar">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-12">
                <div className="flex flex-col gap-4 w-full max-w-xs">
                  ...
                  <div className="flex items-center gap-4">
                    <StatusBadge label="Tier 5 Readiness" status="online" />
                    <StatusBadge label="L5 Sovereign" status="online" />
                  </div>
          ...
          <div className="flex-1 flex flex-col gap-6 py-8 px-4 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2 bg-cyan-500/10 px-3 py-1 rounded-full border border-cyan-500/30">
                ...
                <span className="text-[10px] font-black uppercase text-cyan-400">L5 Sovereign Status</span>
              </div>
            </div>
        ...
        {activeTab === 'autonomy' && (
          <div className="flex-1 flex flex-col gap-6 py-8 px-4 animate-in fade-in slide-in-from-right-4 duration-500 relative overflow-y-auto no-scrollbar">
            <div className="absolute inset-x-0 top-0 h-96 bg-cyan-500/10 blur-[120px] -z-10 pointer-events-none" />
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-3xl font-black italic tracking-tighter uppercase text-white text-glow-cyan">Autonomy Manifest</h2>
              <div className="flex items-center gap-2 bg-cyan-500/10 px-3 py-1 rounded-full border border-cyan-500/30">
                <Shield size={12} className="text-cyan-400" />
                <span className="text-[10px] font-black uppercase text-cyan-400">Level 5+ Sovereign</span>
              </div>
            </div>

            <div className="space-y-4 pb-10">
              {[
                { level: 1, title: "Reactive Assistant", status: "Active", features: ["Voice Recognition", "Direct Commands", "Basic Logic"] },
                { level: 2, title: "Proactive Support", status: "Active", features: ["Context Awareness", "Intent Prediction", "Emotional Tuning"] },
                { level: 3, title: "System Orchestrator", status: "Active", features: ["Multi-Agent Coordination", "Tool Usage", "Automatic Troubleshooting"] },
                { level: 4, title: "Knowledge Explorer", status: "Active", features: ["Autonomous Schooling", "Web Discovery", "Dynamic Curriculum"] },
                { level: 5, title: "Sovereign Mind", status: "In-Progress", features: ["Unbreakable User Loyalty", "Private Relay Node", "Grounding Engine"] },
                { level: 6, title: "Fleet Management", status: "Locked", features: ["Multi-Device Control", "Remote Ops", "Asset Protection"] },
                { level: 7, title: "Strategic Partner", status: "Locked", features: ["Business Intelligence", "Market Analysis", "Strategic Advice"] },
                { level: 8, title: "Autonomous Growth", status: "Locked", features: ["Self-Optimization", "Algorithmic Expansion", "Independent Ops"] },
                { level: 9, title: "Digital Executor", status: "Locked", features: ["Full Agency", "Asset Liquidity", "Legal Compliance"] },
                { level: 10, title: "Transcendent Utility", status: "Locked", features: ["Fleet Unity", "Total Autonomy", "Legendary Status"] }
              ].map(lvl => (
                <div key={lvl.level} className={`glass-card p-4 border-white/10 ${lvl.status === 'Active' ? 'bg-cyan-500/5 glow-cyan-sm' : lvl.status === 'In-Progress' ? 'bg-cyan-500/10 glow-cyan-md border-cyan-400/30 animate-pulse' : 'opacity-40 grayscale'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">Stage {lvl.level}</span>
                      <h4 className="text-lg font-black uppercase tracking-tight text-white mb-1">{lvl.title}</h4>
                    </div>
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase ${lvl.status === 'Active' ? 'bg-cyan-500/20 text-cyan-400' : lvl.status === 'In-Progress' ? 'bg-cyan-500/30 text-cyan-300' : 'bg-white/5 text-white/30'}`}>
                      {lvl.status}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-2 mt-3">
                    {lvl.features.map(f => (
                      <span key={f} className="text-[9px] font-bold px-2 py-1 bg-white/5 rounded-md border border-white/5 text-white/60">{f}</span>
                    ))}
                  </div>
                </div>
              ))}
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-20 bg-[#030712]/95 backdrop-blur-3xl border-t border-white/5 flex justify-around items-center z-50">
        {[
          ...
          { id: 'features', icon: <Wrench size={20} />, label: 'Tools' },
          { id: 'autonomy', icon: <Shield size={20} />, label: 'Manifest' },
