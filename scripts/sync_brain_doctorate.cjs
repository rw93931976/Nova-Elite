    ...
    for (const s of subjects) {
        const { error } = await supabase
            ...
            .upsert({
                ...
                category: 'doctoral_curriculum',
                importance: 10
            }, { onConflict: 'title' });

        if (error) console.error(`Error syncing ${s.name}:`, error.message);
        else console.log(`[Synced] ${s.name}`);
    }

    console.log("✨ Cloud Brain now knows her full Doctorate syllabus.");
}
