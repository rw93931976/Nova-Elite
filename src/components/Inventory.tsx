    ...
    return (
        <div className="pb-32 space-y-12">
            <div className="text-center">
                <h2 className="text-4xl font-black italic tracking-tighter text-[#0BF9EA] mb-2 uppercase drop-shadow-[0_0_20px_rgba(11,249,234,0.3)]">Asset Inventory</h2>
                <p className="text-[#0BF9EA]/40 text-xs font-bold tracking-[0.2em] uppercase">Sovereign Data Assets</p>
            </div>
                    ...
                    <div key={level} className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {CAPABILITIES.filter(c => c.level === level).map((cap, idx) => (
                                <div
                                    key={idx}
                                    className={`${cap.active ? "sovereign-card" : "bg-white/[0.03] border border-white/5 opacity-20 filter grayscale rounded-2xl p-4"} transition-all duration-500`}
                                >
                                    <div className="flex flex-col gap-2">
                                        <div className="flex justify-between items-center">
                                            ...
                                            {cap.active ? <CheckCircle2 size={16} className="text-[#121212]" /> : <Lock size={14} className="text-white/20" />}
                                        </div>
                                    </div>
                                </div>
                            ))}
