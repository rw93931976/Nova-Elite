const WebSocket = require('ws');
require('dotenv').config();

const GOOGLE_API_KEY = process.env.VITE_GOOGLE_AI_KEY;
const wss = new WebSocket.Server({ port: 3505 });

console.log('🚀 [Core] Nova Gemini 2.0 Realtime Live on Port 3505');

wss.on('connection', (ws) => {
    console.log('👤 [Core] Connection Received');

    const url = 'wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1alpha.GenerativeService.BiDiSession?rt=b&key=' + GOOGLE_API_KEY;
    const googleWs = new WebSocket(url);

    googleWs.on('open', () => {
        console.log('✅ [Core] Brain: Connected to Google');
        googleWs.send(JSON.stringify({
            setup: {
                model: "models/gemini-2.0-flash-exp",
                generation_config: { response_modalities: ["audio"] }
            }
        }));
    });

    ws.on('message', (data) => {
        if (googleWs.readyState === WebSocket.OPEN) googleWs.send(data);
    });

    googleWs.on('message', (data) => {
        if (ws.readyState === WebSocket.OPEN) ws.send(data);
    });

    googleWs.on('error', (e) => console.error('❌ Brain Error:', e.message));
    ws.on('error', (e) => console.error('❌ Client Error:', e.message));
    ws.on('close', () => googleWs.close());
    googleWs.on('close', () => ws.close());
});
