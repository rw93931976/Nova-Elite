...
const wss = new WebSocket.Server({ port: 3507, host: '0.0.0.0' });

console.log('🚀 [Core] Nova Gemini 2.0 Realtime Live on Port 3507 (0.0.0.0)');

wss.on('connection', (ws) => {
    console.log('👤 [Core] Connection RECEIVED from Relay');
    ...
    ws.on('message', (data) => {
        if (googleWs.readyState === WebSocket.OPEN) {
            // console.log(`🎤 [Core] Forwarding packet (${data.length} bytes) to Google`);
            googleWs.send(data);
        }
    });
    ...
    ws.on('error', (err) => console.error('❌ [Core] Relay Error:', err.message));
