...
console.log('🛰️ [Relay] Sovereign Relay Live on 127.0.0.1:4500');
...
server.on('upgrade', (request, socket, head) => {
    ...
    wss.handleUpgrade(request, socket, head, (ws) => {
        console.log('🤝 [Relay] Bridge Opening to Core (4001)...');
        const core = new WebSocket('ws://127.0.0.1:4501');
        ...
        core.on('error', (e) => console.error('❌ [Relay] Bridge Error:', e.message));
        core.on('close', (code, reason) => {
            console.log(`⚠️ [Relay] Bridge Closed from Core side (Code: ${code}, Reason: ${reason})`);
            ws.close();
        });
        ws.on('close', () => {
            console.log('👤 [Relay] Browser Client Closed');
            core.close();
        });
    });
});
